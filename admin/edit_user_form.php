...
require('../database.php');
    ...
    //Puts the user that got pick info on the screen in the boxes
...
<body class="backdrop_parallax">
    ...
    <br>
      ...
      <table class = "tableuser">
        ...
        <form id="contactfrm" method="post" action="update_user.php">

        <table id="contactForm" cellpadding="0" cellspacing="8">
          <tr>
            <div>
              <input required type="text" name="id" value="<?php echo $id;?>" pattern="[0-9]{1,2}" class="form-control" placeholder="Id"/>
          </div>
          </tr>

           <tr>
            <div>
              <input required type="text" name="user" value="<?php echo $user;?>" pattern=".{1,20}" class="form-control" placeholder="User"/>
          </div>
          </tr>

          <tr>
            <div>
              <input required type="text" name="password" value="<?php echo $password;?>" pattern=".{6,20}" class="form-control" placeholder="Password (at least six characters long)"/>
          </div>
          </tr>

          <tr>
            <div>
              <input required type="text" name="level" value="<?php echo $level;?>" pattern="[1-2]{1,1}" class="form-control" placeholder="Level (1 or 2)"/>
          </div>
          </tr>   
          <br>
         <tr>
            <input id="update_user_button" type="submit" value="Update"/>
        </tr>
        <br>
        <br>

        <tr>
        <p><a class ="user" href="add_user_form.php" style="color: black;">Add User</a></p><!-- Takes them to the add user page -->
